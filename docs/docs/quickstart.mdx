...
:::tip
You can use the same environment more than once, as Superset persists data locally. However, make sure to properly stop all processes by running the Docker Compose `stop` command. Doing so helps avoid data corruption and data loss.
:::
